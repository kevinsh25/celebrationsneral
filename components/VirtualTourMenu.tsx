...
interface VirtualTourMenuProps {
  onSelect: (tour: 'clubhouse' | 'riverhuts' | 'riverparc') => void;
}
...
export default function VirtualTourMenu({ onSelect }: VirtualTourMenuProps) {
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-12 w-full max-w-7xl h-[65vh] sm:h-[75vh]">
        {/* Ravine Club */}
        <motion.button
        ...
        >
          <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors duration-700 z-10" />
          <Image
            src={`${ASSET_BASE_URL}/RiverParc.webp`}
            alt="River Parc"
            fill
            sizes="(max-width: 768px) 100vw, 33vw"
            className="object-cover transition-transform duration-1000 group-hover:scale-105"
